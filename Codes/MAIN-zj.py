...
def get_sen(e,prev):
    ...
    for k in sorted_cen:
        for (i,lt) in enumerate(e):
            f=0
            if k[0][0] in lt and k[0][1] in lt and vis[i]==0 and k not in prev and len(lt)>8:  ### the length cond added on 15/12 and the 3 hops...before no length cond and 1 hop only
                for item in prev:
                    #print(item)
                    if nx.shortest_path_length(G,source=k[0][0],target=item[0][0])<3: 
...
for levels in range(1,4):
    if levels==1:
        ...
        lev1=get_sen(e,lev1)
        print("Length of lev 1",len(e),e)
    if levels==2:
        e=get_connected(dflev2)
        lev2=get_sen(e,lev1)
        print("Length of lev 2",len(e),e)
    if levels==3:
